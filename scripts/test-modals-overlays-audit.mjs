...
check('Screenshot Tips has an intentional close action instead of circular guide handoff', files.stepAdd.includes('gd-screenshot-tips-secondary') && files.stepAdd.includes('onClick={() => setIsScreenshotTipsOpen(false)}') && !files.stepAdd.includes('onOpenDemo?.(screenshotTipsTriggerRef.current)'));
...
check('Demo arrow navigation is scoped to the guide dialog', files.demo.includes('handleTimelineKeyDown') && files.demo.includes('const activeDialog = target?.closest') && files.demo.includes('activeDialog') && files.demo.includes("activeDialog.getAttribute('aria-labelledby') !== 'demo-modal-title'") && files.demo.includes("window.addEventListener('keydown', handleTimelineKeyDown, { capture: true })"));
