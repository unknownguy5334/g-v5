...
assert(copySrc.includes("title: 'Stop wasting hours between courses.'") || copySrc.includes("title: 'No More Gaps'"), 'Headline is exact.');
assert(copySrc.includes("You don’t choose a 3 hour gap on purpose.") || copySrc.includes("Have you ever had a course at 10 AM and your next one was at 1 PM?"), 'Hero question/line1 is exact.');
assert(copySrc.includes("You just get tired of trying every course combination.") || copySrc.includes("That’s 3 hours stuck on campus, waiting around for your next course."), 'Hero problem statement is exact.');
assert(copySrc.includes("Gadwal doesn’t get tired. It finds the best schedule for you.") || copySrc.includes("Nobody wants to spend their day like that."), 'Hero empathy/resolution statement is exact.');
assert(copySrc.includes("Gadwal doesn’t get tired. It finds the best schedule for you.") || copySrc.includes("Gadwal finds you a schedule that actually makes sense."), 'Hero solution statement is exact and contains no em dash.');
assert(copySrc.toLowerCase().includes("find my best schedule"), 'Primary CTA is exact.');
...
assert(heroBannerSrc.includes('flex flex-col gap-3 max-w-md') || heroBannerSrc.includes('flex flex-col sm:flex-row items-stretch sm:items-center gap-3 max-w-md'), 'Hero actions stack cleanly on small screens.');
assert(heroBannerSrc.includes('px-4 sm:px-6') || heroBannerSrc.includes('px-5 sm:px-6'), 'Hero uses mobile-safe horizontal padding.');
...
assert(howItWorksSrc.includes('COPY.how.title') || howItWorksSrc.includes('How Gadwal works'), 'Detailed explanation exists in the popup modal.');
for (const required of [
  ...
  'no course conflicts',
  'Then you compare the valid options and choose the schedule that works best for you.',
]) {
...
assert(howItWorksSrc.includes('{COPY.how.title}') || howItWorksSrc.includes('How Gadwal works'), 'How-it-works modal reads from the canonical human copy constants.');
