...
const postAssets = Array.from({ length: 7 }, (_, i) => `public/student-posts/post-${String(i + 1).padStart(2, '0')}.webp`);
const durations = [4000, 4000, 5000, 8000, 8000, 11000, 15000];
const exactMarkers = [
...
assert(heroSrc.includes('lg:grid-cols-12') && (heroSrc.includes('lg:col-span-7') || heroSrc.includes('lg:col-span-8') || heroSrc.includes('lg:col-span-6')), 'Hero uses a responsive desktop two-column composition with primary-copy emphasis.');
assert(heroSrc.includes('<StudentGapPosts />') && (heroSrc.includes('lg:col-span-5') || heroSrc.includes('lg:col-span-4') || heroSrc.includes('lg:col-span-6')) && (heroSrc.includes('mt-4 sm:mt-5 lg:mt-0') || heroSrc.includes('mt-4 sm:mt-5 lg:mt-0')), 'Supporting student-post proof stays beside the hero copy on desktop.');
...
assert(order.every((value, index) => value >= 0 && (index === 0 || value > order[index - 1])), 'All seven posts appear in the exact required order.');
...
assert(cssSrc.includes('@media (min-width: 640px)') && cssSrc.includes('.student-gap-post-pagination { display: none; }'), 'Desktop uses minimal carousel navigation without redundant pagination dots.');
assert(cssSrc.includes('grid-template-columns: 1fr;') && cssSrc.includes('.site-footer > div > div:last-child'), 'Mobile footer uses deliberate single-column wrapping.');
assert(carouselSrc.includes('{currentIndex + 1} / {POSTS.length}'), `Progress indicator uses the actual declared post count (${postAssets.length}).`);
