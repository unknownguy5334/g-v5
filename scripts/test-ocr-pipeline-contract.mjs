...
expect(server.includes('const MAX_TOTAL_IMAGES_BYTES = config.maxTotalImagesBytes') && read('server/config.ts').includes('maxTotalImagesBytes: 80 * 1024 * 1024'), 'Server aggregate decoded payload limit must remain explicit.');
...
expect(server.includes('requireAdmin'), 'Provider model discovery must be protected by server-side admin authentication.');
