...
const checks = [
  ...
  [!read('vite.config.ts').includes('allowedHosts: true as const') && read('vite.config.ts').includes('ALLOW_EXTERNAL_VITE_HOST'), 'Vite hosts are restricted by default'],
  ...
  [server.includes('app.get("/api/models", requireAdmin') && config.includes('ENABLE_DIAGNOSTIC_MODELS_ENDPOINT'), 'diagnostic endpoint is hardened'],
  ...
  [pkg.scripts['audit:dependencies']?.includes('test-dependencies-redteam-25.mjs') && pkg.scripts['audit:dependencies']?.includes('npm audit --audit-level=high'), 'dependency audit script exists'],
];
