...
ok(files.add.includes('aria-label={`${section.name || \'Course\'} meeting ${sessIdx + 1} day`}') && files.add.includes('ariaLabel={`${section.name || \'Course\'} meeting ${sessIdx + 1} start time`}') && files.add.includes('ariaLabel={`${section.name || \'Course\'} meeting ${sessIdx + 1} end time`}'), 'OCR review meeting controls have explicit accessible names');
...
ok(files.demo.includes('demo-steps-keyboard-help') && files.demo.includes('Use Left and Right Arrow keys'), 'demo tab keyboard behavior is discoverable');
