...
assert.equal(res21[0].sessions.length, 2);
...
// ============================================================================
// SUITE 9: COMPOSITE BASE COURSE & SECTION CONSOLIDATION (MIU / BIM32108 Pattern)
// ============================================================================
const bimPayload = {
  sections: [
    {
      ...
      credits: 3,
      sessions: [
        { day: 'WED', start: '10:00', end: '11:30', type: 'Lecture' },
        { day: 'THU', start: '10:00', end: '11:30', type: 'Lecture' },
      ],
    },
    {
      id: 'BIM3210801-BUS',
      ...
      credits: 3,
      sessions: [
        { day: 'SAT', start: '08:30', end: '10:00', type: 'Section' },
      ],
    },
  ],
};

const bimAppSections = canonicalToAppSections(modelOutputToCanonical(bimPayload));
const bimReconciled = reconcileOCRSections(bimAppSections);
assert.equal(bimReconciled.length, 1, 'BIM32108-BUS and BIM3210801-BUS must consolidate into exactly 1 course option');
assert.equal(bimReconciled[0].sessions.length, 3, 'Consolidated section must contain all 3 meetings (Wed, Thu, Sat)');
assert.equal(bimReconciled[0].needsReview, false, 'Consolidated section must not be flagged for review');
assert.ok(!bimReconciled[0].reviewReasons?.includes('related_codes_independent_schedules'), 'Must not have related_codes_independent_schedules error');

console.log('Passed Suite 9: Composite Base Course & Section Consolidation');

console.log('\n======================================================');
console.log('ALL SPEC REGRESSION TESTS & SCENARIOS PASSED 100%!');
console.log('======================================================');
