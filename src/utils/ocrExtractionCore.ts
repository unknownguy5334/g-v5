...
export function isLikelyInternalId(id: string | null | undefined): boolean {
  if (!id) return true;
  const clean = String(id).trim();
  if (!clean) return true;
  if (isPlaceholderSectionId(clean)) return true;
  ...
  // Continuous hex hashes (32 hex chars, 64 hex chars, 24 hex mongo objectid)
  if (/^[0-9a-f]{32}$/i.test(clean)) return true;
  if (/^[0-9a-f]{64}$/i.test(clean)) return true;
  if (/^[0-9a-f]{24}$/i.test(clean)) return true;
  // Internal database or system identifier patterns
  if (/^(?:internal|db|sys|system|database|row|auto|gen|generated|storage|pk|rec|record|guid|uuid|temp|session)[-_:]/i.test(clean)) return true;
  if (/(?:internal[-_]db[-_]id|internal[-_]id|db[-_]id|system[-_]id|row[-_]id|auto[-_]id|temp[-_]id)/i.test(clean)) return true;
  if (/^internal\b/i.test(clean)) return true;
  return false;
}

export function extractSectionCodeFromRaw(section: any): string | null {
  if (!section || typeof section !== 'object') return null;

  // 1. section_code
  if (typeof section.section_code === 'string' && section.section_code.trim()) return section.section_code.trim();
  if (typeof section.section_code === 'number' && Number.isFinite(section.section_code)) return String(section.section_code);

  // 2. sectionCode
  if (typeof section.sectionCode === 'string' && section.sectionCode.trim()) return section.sectionCode.trim();
  if (typeof section.sectionCode === 'number' && Number.isFinite(section.sectionCode)) return String(section.sectionCode);

  // 3. section_id
  if (typeof section.section_id === 'string' && section.section_id.trim()) return section.section_id.trim();
  if (typeof section.section_id === 'number' && Number.isFinite(section.section_id)) return String(section.section_id);

  // 4. section_number / sectionNumber
  if (typeof section.section_number === 'string' && section.section_number.trim()) return section.section_number.trim();
  if (typeof section.section_number === 'number' && Number.isFinite(section.section_number)) return String(section.section_number);
  if (typeof section.sectionNumber === 'string' && section.sectionNumber.trim()) return section.sectionNumber.trim();
  if (typeof section.sectionNumber === 'number' && Number.isFinite(section.sectionNumber)) return String(section.sectionNumber);

  // 5. section
  if (typeof section.section === 'string' && section.section.trim()) return section.section.trim();
  if (typeof section.section === 'number' && Number.isFinite(section.section)) return String(section.section);

  // 6. code
  if (typeof section.code === 'string' && section.code.trim()) return section.code.trim();
  if (typeof section.code === 'number' && Number.isFinite(section.code)) return String(section.code);

  // 7. Fallback: inspect `id` when it represents a visible section identifier
  if (typeof section.id === 'string' && section.id.trim()) {
    const rawId = section.id.trim();
    if (!isLikelyInternalId(rawId)) {
      return rawId;
    }
  } else if (typeof section.id === 'number' && Number.isInteger(section.id) && section.id >= 0 && section.id < 100000) {
...
export function getLegacySectionCode(section: any): string | null {
  return extractSectionCodeFromRaw(section);
}
    ...
    const normalizedCourses = raw.courses.map((course: any) => {
      ...
      return {
        ...
        sections: sections.map((s: any) => {
          ...
          const sectionCode = extractSectionCodeFromRaw(s);
          return {
            ...s,
...
function normalizeMeeting(meeting: OCRRawMeeting): { sessions: Session[]; reasons: OCRReviewReason[]; incomplete?: OCRIncompleteMeeting } {
  ...
  const resolvedType = (resolveSessionType(rawType || undefined) || 'Other') as SessionType;
  if (!rawType) reasons.push({ code: 'meeting_type_missing' });
  if (rawType && resolvedType === 'Other' && !/^(?:other|أخرى|اخرى)$/i.test(rawType.trim())) {
    reasons.push({ code: 'meeting_type_unrecognized', detail: rawType });
  }
...
export function modelOutputToCanonical(rawParsed: any, source: OcrSourceMeta = {}): CanonicalOCRSection[] {
  ...
  for (const rawCourse of normalizedInput.courses as OCRRawCourse[]) {
    ...
    if (!courseName && !courseCode) baseReasons.push({ code: 'course_name_missing' });
...
export function canonicalToAppSections(items: CanonicalOCRSection[]): Section[] {
  return items.map((item) => ({
    ...
    sectionRelationship: item.sectionRelationship as Section['sectionRelationship'],
...
export function validateOCRSections(sections: Section[]): { valid: boolean; score: number; reasons: string[] } {
  ...
  for (const section of sections) {
    ...
    for (const session of section.sessions || []) {
      if (session.day && !DAY_ORDER.includes(session.day)) { reasons.push('invalid_day'); invalidNormalizedSession = true; continue; }
