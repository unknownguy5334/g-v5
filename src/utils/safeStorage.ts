import { devLogWarn } from './clientLogger';
...
function getStore(kind: 'local' | 'session'): StorageLike | null {
  try {
    ...
    devLogWarn(`safeStorage: Could not access ${kind}Storage.`);
    return null;
  }
}
...
function getItem(kind: 'local' | 'session', key: string): string | null {
  try {
    ...
    devLogWarn(`safeStorage: Could not read ${kind} key "${key}".`);
    return null;
  }
}
...
function setItem(kind: 'local' | 'session', key: string, value: string): boolean {
  try {
    ...
    devLogWarn(`safeStorage: Could not write ${kind} key "${key}".`);
    return false;
  }
}
...
function removeItem(kind: 'local' | 'session', key: string): boolean {
  try {
    ...
    devLogWarn(`safeStorage: Could not remove ${kind} key "${key}".`);
    return false;
  }
}
