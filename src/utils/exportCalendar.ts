...
/** RFC 5545 line folding: fold long logical lines at 75 octets without splitting UTF-8 code points. */
function foldIcsLine(line: string): string[] {
  const encoder = typeof TextEncoder !== 'undefined' ? new TextEncoder() : null;
  const byteLength = (value: string) => encoder ? encoder.encode(value).length : unescape(encodeURIComponent(value)).length;
  if (byteLength(line) <= 75) return [line];
  const chunks: string[] = [];
  let current = '';
  for (const char of line) {
    const next = current + char;
    const allowance = chunks.length === 0 ? 75 : 74;
    if (byteLength(next) > allowance && current) {
      chunks.push(current);
      current = ' ' + char;
    } else {
      current = next;
...
function collectScheduleConflicts(schedule: OptimizationResult): string | null {
  const seenCourseKeys = new Set<string>();
  const byDay: Record<string, Array<{ start: number; end: number; name: string }>> = {};
  for (const section of schedule.sections) {
    const courseKey = String(section.courseKey || section.courseCode || section.name || '').trim().toLowerCase();
    if (courseKey && seenCourseKeys.has(courseKey)) {
      return `Duplicate course in schedule: ${section.name || courseKey}.`;
    }
    if (courseKey) seenCourseKeys.add(courseKey);
    for (const session of section.sessions) {
      const start = parseExportTime(session.start);
      const end = parseExportTime(session.end);
      if (!start || !end) continue;
      const dayList = byDay[session.day] || (byDay[session.day] = []);
      for (const existing of dayList) {
        if (start.hours * 60 + start.minutes < existing.end && existing.start < end.hours * 60 + end.minutes) {
          return `Course conflict on ${session.day} between ${existing.name} and ${section.name || 'another course'}.`;
        }
      }
      dayList.push({ start: start.hours * 60 + start.minutes, end: end.hours * 60 + end.minutes, name: section.name || 'Unnamed course' });
    }
  }
  return null;
}
...
export function assertExportableSchedule(schedule: OptimizationResult): void {
  ...
  for (const section of schedule.sections) {
    if (!section || typeof section.name !== 'string' || !section.name.trim() || !Array.isArray(section.sessions) || section.sessions.length === 0) {
    ...
    for (const session of section.sessions) {
      ...
      if (session.ambiguousTime && !session.resolvedFromAmbiguousTime) {
        throw new Error(`Cannot export an unresolved ambiguous time for ${section.name}.`);
      }
    }
  }
  const conflict = collectScheduleConflicts(schedule);
  if (conflict) throw new Error(`Cannot export an invalid schedule: ${conflict}`);
}
