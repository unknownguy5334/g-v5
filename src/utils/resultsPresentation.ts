...
export function getRankingSummary(
  ...
  if (!previous) return `Valid option for ${schedule.numDays} ${schedule.numDays === 1 ? 'day' : 'days'}`;
  return `Valid alternative for ${schedule.numDays} ${schedule.numDays === 1 ? 'day' : 'days'}`;
}
