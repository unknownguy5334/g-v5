  ...
  for (const incoming of incomingSections || []) {
    ...
    if (existingIndex >= 0) {
      ...
      const updated: Section = {
        ...
        sectionCodeMissing: !((existing.sectionCode && !existing.sectionCodeMissing) || incomingCode),
      };
...
export function getInputsChangeSummary(
  ...
  if (JSON.stringify(prevPrefValue) !== JSON.stringify(currPrefValue)) {
    hasPreferenceChange = true;
    if (prevPrefValue.targetCredits !== currPrefValue.targetCredits) reasons.push('Target credits changed');
    if (prevPrefValue.targetCourseCount !== currPrefValue.targetCourseCount) reasons.push('Target course count changed');
    if (prevPrefValue.mandatoryCourseKeys.join('|') !== currPrefValue.mandatoryCourseKeys.join('|')) reasons.push('Mandatory courses selection was updated');
    if (prevPrefValue.mandatoryCourses.join('|') !== currPrefValue.mandatoryCourses.join('|')) reasons.push('Mandatory course labels changed');
  }
