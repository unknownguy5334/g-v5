...
export async function callOcrModelWithRetry<T>(
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      return await generateWithTimeout(generate, options.timeoutMs, options.parentSignal);
    } catch (err) {
      lastError = err;
      if (options.parentSignal?.aborted) throw new Error('CLIENT_ABORTED');
      if (!isOcrRateLimitError(err) || attempt >= attempts - 1) throw err;

      const base = extractRetryDelayMs(err) ?? 1500;
      const jitter = Math.max(0.8, Math.min(1.2, 0.8 + random() * 0.4));
      const retryDelayMs = Math.min(8000, Math.max(500, Math.round(base * jitter)));
      if (options.parentSignal) {
        await new Promise<void>((resolve, reject) => {
          ...
          const cleanup = () => options.parentSignal?.removeEventListener('abort', onAbort);
          options.parentSignal?.addEventListener('abort', onAbort, { once: true });
          Promise.resolve(sleep(retryDelayMs)).then(() => { cleanup(); resolve(); }, (sleepError) => { cleanup(); reject(sleepError); });
        });
      } else {
...
function hasSafeJsonNesting(candidate: string, maxDepth = 32): boolean {
  let depth = 0;
  let inString = false;
  let escaped = false;
...
export function parseOcrJsonPayload(responseText: unknown): Record<string, unknown> | null {
  ...
  const tryParse = (candidate: string): Record<string, unknown> | null => {
    if (candidate.length > MAX_SCAN_CHARS || !hasSafeJsonNesting(candidate)) return null;
    try {
...
export function validateOcrModelShape(value: unknown): { valid: boolean; reasons: string[] } {
  ...
  const MAX_TOTAL_SECTIONS = 500;
  const MAX_MEETINGS_PER_SECTION = 32;
  const MAX_REASON_ITEMS = 50;
  ...
  const isStringArray = (v: unknown, maxItems = MAX_REASON_ITEMS) => v === undefined || (Array.isArray(v) && v.length <= maxItems && v.every((x) => typeof x === 'string'));
  if (root.courses.length > MAX_COURSES) reasons.push('courses_too_many');
  ...
  let totalSections = 0;
  root.courses.slice(0, MAX_COURSES).forEach((course, index) => {
    ...
    if ('credit_hours_conflict' in c && c.credit_hours_conflict !== null && !(Array.isArray(c.credit_hours_conflict) && c.credit_hours_conflict.length <= MAX_REASON_ITEMS && c.credit_hours_conflict.every((x) => typeof x === 'number' && Number.isFinite(x)))) reasons.push(`${path}_credit_hours_conflict_invalid`);
    ...
    if (sections.length > MAX_TOTAL_SECTIONS - totalSections) reasons.push(`${path}_sections_too_many`);
    sections.slice(0, Math.max(0, MAX_TOTAL_SECTIONS - totalSections)).forEach((section, sectionIndex) => {
      totalSections += 1;
      ...
      if (!Array.isArray(s.meetings)) reasons.push(`${spath}_meetings_array_missing`);
        ...
        if (s.meetings.length > MAX_MEETINGS_PER_SECTION) reasons.push(`${spath}_meetings_too_many`);
        s.meetings.slice(0, MAX_MEETINGS_PER_SECTION).forEach((m, mi) => checkMeeting(m, `${spath}_meeting_${mi}`));
      }
      if ('incomplete_meetings' in s) {
        if (!Array.isArray(s.incomplete_meetings)) reasons.push(`${spath}_incomplete_meetings_invalid`);
        else {
          if (s.incomplete_meetings.length > MAX_MEETINGS_PER_SECTION) reasons.push(`${spath}_incomplete_meetings_too_many`);
          s.incomplete_meetings.slice(0, MAX_MEETINGS_PER_SECTION).forEach((m, mi) => checkMeeting(m, `${spath}_incomplete_meeting_${mi}`));
        }
      }
      if ('conflicting_meetings' in s) {
        if (!Array.isArray(s.conflicting_meetings)) reasons.push(`${spath}_conflicting_meetings_invalid`);
        else {
          if (s.conflicting_meetings.length > MAX_MEETINGS_PER_SECTION) reasons.push(`${spath}_conflicting_meetings_too_many`);
          s.conflicting_meetings.slice(0, MAX_MEETINGS_PER_SECTION).forEach((m, mi) => checkMeeting(m, `${spath}_conflict_${mi}`));
        }
      }
    });
  });
...
export function validateOcrApiResponse(value: unknown): { valid: true; status: OcrApiResponseStatus; sections: Section[]; reasonCode: string; ocrRunId?: string } | { valid: false; reason: string } {
  ...
  for (const raw of data.sections) {
    ...
    const normalizedSessions = section.sessions.map((session) => {
      ...
      return { ...normalized, day: String(normalized.day || ''), start: String(normalized.start || ''), end: String(normalized.end || ''), type: normalized.type, customType: normalized.customType };
    }).filter((session): session is NonNullable<typeof session> => session !== null);
    for (const session of normalizedSessions) {
      if (session.day !== '' && !['SAT','SUN','MON','TUE','WED','THU','FRI'].includes(session.day)) return { valid: false, reason: 'A returned session contains an invalid day.' };
...
export function buildOcrRequest(images: OcrImageRequest[], workflowGenerationId: string): OcrRequest {
  if (!Array.isArray(images) || images.length === 0 || images.length > 30) throw new Error('Invalid OCR request image count.');
  if (typeof workflowGenerationId !== 'string' || workflowGenerationId.trim().length === 0 || workflowGenerationId.length > 160 || /[\u0000-\u001F\u007F]/.test(workflowGenerationId)) {
    throw new Error('Invalid OCR workflow generation identifier.');
  }
  const validMimeTypes = new Set(['image/png', 'image/jpeg', 'image/webp'] as const);
  return {
    images: images.map((image) => {
      if (!image || typeof image.data !== 'string' || image.data.length === 0 || image.data.length > 20_000_000 || !validMimeTypes.has(image.mimeType)) {
        throw new Error('Invalid OCR image payload.');
      }
      return { data: image.data, mimeType: image.mimeType };
    }),
    workflowGenerationId: workflowGenerationId.trim(),
  };
}
