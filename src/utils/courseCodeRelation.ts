...
export function classifySectionCodeRelation(
  ...
  if (suffix) {
    ...
    const suffixRegex = new RegExp(`[\\s_-]*${escapeRegexLiteral(suffix)}$`, 'i');
...
export function detectCompositeDerivedCode(
  ...
  for (let baseLen = 2; baseLen <= cleanShorter.length; baseLen++) {
    ...
    if (candidateSuffix) {
      const suffixPattern = new RegExp(`[\\s_-]*${escapeRegexLiteral(candidateSuffix)}$`, 'i');
...
export function areSchedulesCompatibleForDerivedMerge(
  ...
  if (sessionsA.length === 0 || sessionsB.length === 0) {
    return { compatible: true, reason: 'empty_schedule_evidence' };
  }
  ...
  // 1. Exact duplicate meetings
  if (setA.size === setB.size && [...setA].every((k) => setB.has(k))) {
    return { compatible: true, reason: 'exact_duplicate_meetings' };
  }

  // 2. Subset / Superset: one record contains a subset of meetings of the other
  const isASubsetOfB = [...setA].every((k) => setB.has(k));
  const isBSubsetOfA = [...setB].every((k) => setA.has(k));
  if (isASubsetOfB || isBSubsetOfA) {
    return { compatible: true, reason: 'subset_schedule' };
  }

  // 3. Complementary meetings (e.g. shared lectures + specific section/lab, or non-overlapping meeting times)
  let hasTimeConflict = false;
  for (const a of sessionsA) {
    if (!a.day || !a.start || !a.end) continue;
    const aStart = Number(String(a.start).slice(0, 2)) * 60 + Number(String(a.start).slice(3));
    const aEnd = Number(String(a.end).slice(0, 2)) * 60 + Number(String(a.end).slice(3));
    for (const b of sessionsB) {
      if (!b.day || !b.start || !b.end) continue;
      if (String(a.day).toUpperCase() !== String(b.day).toUpperCase()) continue;
      const bStart = Number(String(b.start).slice(0, 2)) * 60 + Number(String(b.start).slice(3));
      const bEnd = Number(String(b.end).slice(0, 2)) * 60 + Number(String(b.end).slice(3));
  ...
  if (!hasTimeConflict) {
    return { compatible: true, reason: 'complementary_sessions' };
  }
