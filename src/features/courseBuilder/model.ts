...
export const createEmptySession = (idSuffix: string = '1', defaultType: SessionType | '' = ''): ManualSessionRow => ({
...
export function sanitizeRecoveredSession(value: unknown): ManualSessionRow | null {
  ...
  const norm = normalizeMeetingType(s.type);
  return {
    id: s.id.trim(),
    ...
    type: norm.type,
    customType: norm.customType ?? (typeof s.customType === 'string' ? s.customType : ''),
  };
}
...
export function sanitizeRecoveredManualForm(value: unknown): ManualFormState | null {
  ...
  return {
    ...
    sectionCode: typeof f.sectionCode === 'string' ? f.sectionCode : (typeof f.courseCode === 'string' ? f.courseCode : ''),
    credits: f.credits,
...
export function sanitizeRecoveredSections(value: unknown): Section[] {
  ...
  return value.flatMap((valueItem) => {
    ...
    const sessions = item.sessions.flatMap((sessionValue) => {
      ...
      const norm = normalizeMeetingType(session.type);
      return [{
        id: typeof session.id === 'string' ? session.id.trim() : undefined,
        day: day as Section['sessions'][number]['day'], start, end,
        type: norm.type,
        customType: norm.customType ?? (typeof session.customType === 'string' ? session.customType.slice(0, 200) : undefined),
        ...
        sourceEvidence: session.sourceEvidence && typeof session.sourceEvidence === 'object' ? {
          sourceImageIndexes: Array.isArray((session.sourceEvidence as Record<string, unknown>).sourceImageIndexes) ? ((session.sourceEvidence as Record<string, unknown>).sourceImageIndexes as unknown[]).filter((n): n is number => typeof n === 'number' && Number.isInteger(n) && n >= 0).slice(0, 30) : undefined,
...
export function sanitizePendingReviewSections(value: unknown): Section[] {
  ...
  return value.flatMap((valueItem) => {
    ...
    return [{
      ...
      userOverrides: item.userOverrides && typeof item.userOverrides === 'object' && !Array.isArray(item.userOverrides) ? Object.fromEntries(Object.entries(item.userOverrides as Record<string, unknown>).filter(([,v]) => ['string','number','boolean'].includes(typeof v) || v === null).slice(0,30)) as Section['userOverrides'] : undefined,
      sourceEvidence: { sourceImageIndexes: Array.isArray(item.sourceImageIndexes) ? item.sourceImageIndexes.filter((n): n is number => typeof n === 'number' && Number.isInteger(n) && n >= 0).slice(0,30) : [], ocrRunId: typeof item.ocrRunId === 'string' ? item.ocrRunId.slice(0,100) : undefined },
    } as Section];
