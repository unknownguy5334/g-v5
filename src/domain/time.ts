...
export function timeToMinutes(raw: string | null | undefined): number {
  ...
  if (!match) {
    const normalized = normalizeTimeWithPolicy(value);
    if (normalized) match = /^(\d{1,2}):(\d{2})$/.exec(normalized);
  }
  if (!match) return NaN;
...
export function normalizeTimeToHHMM(raw: string | null | undefined): string | null {
  const m = timeToMinutes(raw);
  return Number.isFinite(m) ? minutesToTime(m) : null;
}
