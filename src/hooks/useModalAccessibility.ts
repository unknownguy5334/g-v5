...
function refreshBackgroundInert() {
  ...
  // regions that need isolation. The modal lives in the dedicated overlay root or
  // inside an active dialog container.
  const protectedTopLevel = new Set<Element>();
  if (overlayRoot) protectedTopLevel.add(overlayRoot);

  // Protect any container or DOM ancestor that holds the active modal dialog so it remains fully clickable and scrollable.
  const activeDialogs = document.querySelectorAll<HTMLElement>('[data-gadwal-overlay-id], [role="dialog"], .gd-modal-backdrop');
  activeDialogs.forEach((dialog) => {
    let curr: HTMLElement | null = dialog;
    while (curr && curr !== document.body) {
      protectedTopLevel.add(curr);
  ...
  for (const node of bodyChildren) {
    ...
    if (protectedTopLevel.has(node)) {
      if (inertedElements.has(element)) {
        const snapshot = inertedElements.get(element)!;
        setElementInert(element, snapshot.inert);
        if (snapshot.ariaHidden === null) element.removeAttribute('aria-hidden');
        else element.setAttribute('aria-hidden', snapshot.ariaHidden);
...
function unlockScroll() {
  ...
  if (activeLockCount === 0) {
    ...
    if (previousBodyPosition === '') window.scrollTo(0, previousScrollY);
  }
}
