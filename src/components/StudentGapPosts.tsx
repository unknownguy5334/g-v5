...
export const StudentGapPosts: React.FC = () => {
  ...
  const goTo = useCallback((index: number) => {
    ...
    clearTimer();
    remainingMsRef.current = POSTS[index].durationMs;
    totalDurationRef.current = POSTS[index].durationMs;
    startedAtRef.current = null;
    setProgressPercent(0);
  ...
  return (
    <section
      className="w-full max-w-[560px] mx-auto lg:mr-0"
      aria-label="Student posts about schedule gaps"
    >
      <div className="mb-2 sm:mb-2.5 flex items-end justify-between gap-3">
        <div>
          <p className="text-[11px] sm:text-xs font-black uppercase tracking-[0.16em] text-text-muted">
            Student posts
          </p>
          <p className="mt-0.5 text-sm sm:text-base font-semibold text-ink">
            We've all had days like this.
          </p>
          <p className="mt-0.5 text-xs sm:text-sm text-text-secondary leading-relaxed max-w-md">
            Stories about the time students lose between courses.
          </p>
        </div>
        <span className="shrink-0 text-xs font-mono font-bold text-text-muted px-2 py-0.5 bg-mist/60 rounded border border-line/60" aria-label={`Post ${currentIndex + 1} of ${POSTS.length}`}>
          {currentIndex + 1} / {POSTS.length}
        </span>
      </div>

      <div
        className="student-gap-posts group relative aspect-[16/10] min-h-[190px] max-h-[310px] flex items-center justify-center overflow-hidden rounded-2xl border border-line-strong bg-white px-3 pt-3 pb-1.5 sm:px-4 sm:pt-3.5 sm:pb-2 shadow-2xs"
      ...
      >
        ...
        <img
          ...
          src={post.src}
          alt={post.alt}
          className="student-gap-post-image block w-auto max-w-full max-h-[195px] sm:max-h-[225px] lg:max-h-[240px] object-contain rounded-lg mx-auto shadow-2xs"
          draggable={false}
        />
      </div>

      <div className="mt-2.5 flex items-center justify-between">
        <span className="text-[11px] sm:text-xs text-text-muted font-medium select-none">
          {isPaused ? 'Paused while reading' : 'Stories from students'}
        </span>
        <div className="student-gap-post-pagination hidden" role="group" aria-label="Student post selection">
          {POSTS.map((item, index) => (
            <button
              key={item.src}
              type="button"
              aria-label={`Show student post ${index + 1} of ${POSTS.length}`}
              onClick={() => goTo(index)}
              className="inline-flex items-center justify-center w-10 h-10 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
            >
              <span className={`h-1.5 rounded-full transition-all duration-200 ${index === currentIndex ? 'w-5 bg-ink' : 'w-1.5 bg-line-strong'}`} aria-hidden="true" />
            </button>
          ))}
        </div>
        <div className="student-gap-post-nav-buttons flex items-center justify-end gap-2">
          <button
            type="button"
            aria-label="Previous student post"
            onClick={() => advance(-1)}
            className="min-h-[44px] px-3 rounded-lg min-w-[44px] border border-line-strong bg-white text-ink hover:bg-mist focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent transition active:translate-y-px inline-flex items-center justify-center cursor-pointer shadow-2xs"
          >
            <ArrowLeft className="h-4 w-4 sm:h-4.5 sm:w-4.5" aria-hidden="true" />
          </button>
          <button
            type="button"
            aria-label="Next student post"
            onClick={() => advance(1)}
            className="min-h-[44px] px-3.5 rounded-lg min-w-[44px] border border-line-strong bg-white text-ink hover:bg-mist focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent transition active:translate-y-px inline-flex items-center justify-center cursor-pointer shadow-2xs"
          >
            <ArrowRight className="h-4 w-4 sm:h-4.5 sm:w-4.5" aria-hidden="true" />
          </button>
        </div>
      </div>
    </section>
