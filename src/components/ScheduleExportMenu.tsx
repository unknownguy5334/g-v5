...
import { Download, FileText, Image as ImageIcon, Copy, Check, ChevronDown, Loader2 } from 'lucide-react';
...
export const ScheduleExportMenu: React.FC<ScheduleExportMenuProps> = ({ schedule, rank }) => {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      if (err?.name !== 'AbortError') {
        devLogError('Image export failed.');
  ...
  return (
    <div className="relative inline-block text-left" ref={menuRef}>
      <button
        ...
        aria-label={`Export options for schedule #${rank}`}
        ...
        >
          <div className="py-1">
            ...
            <button tabIndex={0} type="button" role="menuitem" disabled={loadingAction !== null} onClick={handleDownloadPDF} className="w-full text-left px-3.5 py-2.5 min-h-[44px] text-sm font-medium text-ink hover:bg-mist flex items-center gap-2.5 transition cursor-pointer disabled:opacity-40">
              {loadingAction === 'pdf' ? <Loader2 className="w-4 h-4 animate-spin text-ink"/> : <FileText className="w-4 h-4 text-ink" aria-hidden="true"/>}
            ...
            <button tabIndex={0} type="button" role="menuitem" disabled={loadingAction !== null} onClick={handleDownloadImage} className="w-full text-left px-3.5 py-2.5 min-h-[44px] text-sm font-medium text-ink hover:bg-mist flex items-center gap-2.5 transition cursor-pointer disabled:opacity-40">
              {loadingAction === 'image' ? <Loader2 className="w-4 h-4 animate-spin text-ink"/> : <ImageIcon className="w-4 h-4 text-ink" aria-hidden="true"/>}
            ...
            <button
              ...
              onClick={async () => {
                try {
                  ...
                  const result = await copyToClipboardDetailed(text);
                  if (result.success) {
                    setIsCopied(true);
                    if (copiedTimerRef.current !== null) window.clearTimeout(copiedTimerRef.current);
                    ...
                    setErrorMessage('Could not copy automatically. Please try again.');
                  }
                } catch (err) {
                  devLogError('Copy text failed.');
            ...
            >
              {isCopied ? <Check className="w-4 h-4 text-emerald-600" aria-hidden="true" /> : <Copy className="w-4 h-4 text-ink" aria-hidden="true" />}
              <span className={isCopied ? 'text-emerald-700 font-bold' : ''}>
                {isCopied ? 'Copied to clipboard!' : 'Copy as text'}
              </span>
            </button>
          </div>
