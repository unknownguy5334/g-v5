...
import { safeStorage, parseStorageEnvelope, createStorageEnvelope } from '../utils/safeStorage';
...
export const DEFAULT_PREFERENCES: SchedulePreferences = { targetCredits: null, targetCourseCount: null, mandatoryCourses: [], mandatoryCourseKeys: [] };
  ...
  const computeScheduleMetrics = (scheduleSections: Section[]) => {
    ...
    for (const sessions of Object.values(daySessions)) {
      ...
      for (let i = 0; i < sessions.length; i++) {
        if (Number.isFinite(previousEnd) && sessions[i].start > previousEnd) {
          totalGap += (sessions[i].start - previousEnd);
        }
        previousEnd = Number.isFinite(previousEnd) ? Math.max(previousEnd, sessions[i].end) : sessions[i].end;
      }
    }
