...
export function savePendingReview(envelope: PendingReviewEnvelope, options: { durable?: boolean } = {}): PersistenceWriteResult {
  ...
  const durableSaved = options.durable !== false && safeStorage.setItem(WORKFLOW_DRAFT_KEYS.pendingReview, payload);
...
export function saveManualForms(forms: ManualFormState[], options: { durable?: boolean } = {}): PersistenceWriteResult {
  ...
  const durableSaved = options.durable !== false && safeStorage.setItem(WORKFLOW_DRAFT_KEYS.manualForms, payload);
